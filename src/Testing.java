...
public class Testing {
	...
	static boolean statChange = true;
	...
	public static void main(String[] args) {
		...
		System.out.println("");
		while (turnCount < 3) { // Enables the game to loop for the number of turns that is required for testing.
		...
		System.out.println(charName + ", you survived encounters with " + (turnCount) + " creatures.");
...
//		TEST #1 
//		Aggressive creature with 3,2,4
		if (turnCount == 1) { 
			creatureSpeed = 3;
			creatureWep = 2;
			creatureInt = 4;
			creatureState = creatureArray[2];
		}
//		Curious creature 3,2,4
		if (turnCount == 2) { 
			creatureSpeed = 3;
			creatureWep = 2;
			creatureInt = 4;
			creatureState = creatureArray[1];
		}
//		Scared creature 3,2,4
		if (turnCount == 3) { 
			creatureSpeed = 3;
			creatureWep = 2;
			creatureInt = 4;
			creatureState = creatureArray[0];
		}
...
//   	Test #2
//		
//		Aggressive creature with 5,5,5
		if ( turnCount == 1) { 
			creatureSpeed = 5;
			creatureWep = 5;
			creatureInt = 5;
...
//		Aggressive creature with 5,1,5
		if (turnCount == 4) { 
			creatureSpeed = 5;
			creatureWep = 1;
			creatureInt = 5;
...
//		Aggressive creature with 5,5,3
		if ( turnCount == 2) { 
			creatureSpeed = 5;
			creatureWep = 5;
			creatureInt = 3;
...
//		Aggressive creature with 4,5,5
		if (turnCount == 3) { 
			creatureSpeed = 4;
			creatureWep = 5;
			creatureInt = 5;
...
*/
	...
	public void chooseSpecial() { // This will work out what special ability is best to use against the current
		...
		// Computer working out which special is the best to use
		if ((textbookWin == true && noOfTextbooks > 0)
				&& ((intDifference + textbookPow) >= (wepDifference + flashgunPow))) {
			choice = "textbook";
		} else if ((flashgunWin == true && noOfFlashguns > 0)
				&& ((wepDifference + flashgunPow) >= (intDifference + textbookPow))) {
			choice = "flashgun";
...
//			else if ((noOfFlashguns > 0) // If none of the above options are possible then the ai should try to use a
//											// special.
...
//				creatureChange = true;
//			} 
			else if (wepDifference == 1 || intDifference == 1 || wepDifference == -1 || intDifference == -1) { // With the curious monster if the AI cannot kill it
		...
		if (creatureState.equals(creatureArray[2])) { // if creature is aggressive the AI will go through this choice
			...
			if (wepDifference >= 2 || wepDifference == 0) { // 100% chance user victory (AI wins or creature runs away
				...
				// This is the last thing the AI will choose to do asit only gives a 50% chance of victory the AI will
	...
	public void resolveFight() { // If resolveFight() is called then this is where the program will depend who the winner of the fight is.
